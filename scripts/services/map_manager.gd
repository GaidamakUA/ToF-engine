...
var skirmish: Dictionary[String, Dictionary] = {}
var special: Dictionary[String, Dictionary] = {}
var online: Dictionary[String, Dictionary] = {}
...
func load_list_from_file() -> void:
    self.maps.assign(self.filesystem.read_json_from_file(self.LIST_FILE_PATH) as Dictionary)
    self.online.assign(self.filesystem.read_json_from_file(self.ONLINE_FILE_PATH) as Dictionary)
...
func _load_bundled_skirmish_maps() -> void:
    var loaded_maps: Array[String] = self._load_map_names_from_dir(self.BUNDLED_MAP_PATH)
...
func _load_bundled_special_maps() -> void:
    var loaded_maps: Array[String] = self._load_map_names_from_dir(self.SPECIAL_MAP_PATH)
    for map_name: String in loaded_maps:
...
func _load_map_names_from_dir(dir_path: String) -> Array[String]:
    var names: Array[String] = []

    if self.filesystem.dir_exists(dir_path):
        var file_list: Array[String] = self.filesystem.dir_list(dir_path, true)
...
func get_map_data(map_name: String) -> Dictionary:
    ...
    return self.filesystem.read_json_from_file(filepath) as Dictionary
