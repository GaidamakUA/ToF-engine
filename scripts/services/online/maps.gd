...
func fetch_listing_chunk(last_id: int) -> int:
	...
	var response = await self.online_service.connector._get_request(resource, true)
...
func upload_map(map_data: Dictionary) -> Dictionary:
	...
	var response = await self.online_service.connector._post_request(self._base_url(), serialized_json)
