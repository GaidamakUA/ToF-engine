...
func test_move_unit_along_path_relocates_unit_spends_ap_and_emits_event() -> void:
	...
	assert_eq((result.events[0] as ApChangedEvent).old_ap, 4)
	assert_eq((result.events[0] as ApChangedEvent).new_ap, 2)
	assert_true(result.events[1] is UnitMovedEvent)
	assert_same((result.events[1] as UnitMovedEvent).start, source)
	assert_same((result.events[1] as UnitMovedEvent).finish, destination)
