...
func shoot_at_position(destination_position: Vector3, tween_time: float) -> void:
...
func lob_at_position(destination_position: Vector3, tween_time: float) -> void:
    self.look_at(destination_position, Vector3(0, 1, 0))

    create_tween().tween_property(self, "position", destination_position, tween_time).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_LINEAR)

    var mesh_translation: Vector3 = self.mesh.get_position()
